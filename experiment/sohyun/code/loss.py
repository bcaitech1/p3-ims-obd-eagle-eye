...
class CompoundLoss(nn.Module):
    ...
    def get_gt_proportion(
        self, labels: torch.Tensor, target_shape, ignore_index: int = 255
    ):
    ...
    def get_pred_proportion(
        self, logits: torch.Tensor, temp: float = 1.0, valid_mask=None
    ):
...
class CrossEntropyWithL1(CompoundLoss):
    ...
    def forward(self, inputs: torch.Tensor, labels: torch.Tensor):
        ...
        pred_proportion = self.get_pred_proportion(
            inputs, temp=self.temp, valid_mask=valid_mask
        )
